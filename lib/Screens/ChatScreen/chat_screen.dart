  ...
  void markMessagesAsRead(
    ...
    for (var message in messages) {
      if (message.senderId != readerId) {
        if (message.isRead?[searcherId] == false ||
            message.isRead?[companyId] == false) {
                    ...
                    onPressed: () async {
                      if (_messageController.text.isNotEmpty) {
                        ...
                        if (chatProvider.companyIsConect == false) {
                          final companyId = widget.companyModel?.id.toString();
                          await SendNotificationsService
                              .sendNotificationForSingleUserUsingApi(
                                  token: widget.companyModel?.tokenNotification,
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        MessageModel message = messages[index];
                        bool isRead = message.isRead?[widget.readerId] ?? false;
                        return MessageBubble(
                            time: chatProvider
                                .formatTimestamp(message.timestamp!),
                            subtitle: (message.senderId != widget.readerId)
